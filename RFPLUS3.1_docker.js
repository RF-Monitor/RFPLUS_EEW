...
function handleDisconnect_conn2() {
    ...
    conn2.on('error', (err) => {
        console.error('MySQL pga DB error:', err);
        ...
        if (reconnectErrors.includes(err.code)) {
        console.warn(`Attempting to reconnect after error: ${err.code}`);
        setTimeout(handleDisconnect_conn2, 2000); // 延遲2秒重連，避免頻繁嘗試
        } else {
        console.error('Unrecoverable MySQL error, manual intervention required.');
        console.error(err);
        }
    });
}
